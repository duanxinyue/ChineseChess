...
function buildBlobWorkerSource(id, bundle) {
  ...
  if (id === "pikafish") {
    return [
      ...
      "var engine=null,lastSeq=0,pendingSearch=null,stopPending=false;",
      ...
      "  if(stopPending){stopPending=false;return;}",
      "  var p=line.split(/\\s+/);self.postMessage({type:'BEST_MOVE',move:(p.length>1?p[1]:''),seq:lastSeq});}",
      "}",
      "function stopSearch(){try{if(engine){engine.sendCommand('stop');}}catch(e0){}}",
      "self.onmessage=function(e){var d=e.data||{};",
      " if(d.type==='STOP'){stopPending=true;stopSearch();pendingSearch=null;return;}",
  ...
  return [
    ...
    "self.onmessage=function(e){var d=e.data||{};",
    " if(d.type==='STOP'){stopPending=true;cmd('stop');pendingSearch=null;return;}",
