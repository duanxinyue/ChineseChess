/*
engine.js - 皮卡鱼(Pikafish)引擎桥接层

唯一引擎:
  "pikafish" 皮卡鱼(Pikafish, WASM + NNUE, UCI/UCCI 协议, Worker 后台搜索)

Worker 双通道:
  * 原生 Worker(http(s) 下可用, 直接加载 js/engines/pikafish/ 下的引擎文件)
...
对外接口:
  ...
  EngineBridge.stop(id)
  EngineBridge.unload(id)
  EngineBridge.reset(id)             // 销毁 Worker, 下次搜索自动重载
  EngineBridge.supported(id) -> bool
  EngineBridge.displayName(id) -> string
  EngineBridge.ready(id) -> bool
  EngineBridge.mode(id) -> "native" | "blob" | null
  EngineBridge.setRuleOptions({ allowChase })
*/
"use strict";

var ENGINE_LIST = [
  { id: "pikafish", name: "皮卡鱼 Pikafish（NNUE）", wasm: true,
...
// 生成自包含的皮卡鱼 Blob Worker 源码(无任何外部文件依赖)
function buildBlobWorkerSource(bundle) {
...
var EngineBridge = (function () {
  ...
  var searches = {};
  var seqAlloc = 0;
  // 判例开关: 随每次搜索下发, 由 worker 在搜索前同步到引擎
  ...
  function spawnWorker(id) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (typeof Worker == "undefined") {
        reject(new Error("当前环境不支持 Web Worker，无法使用引擎"));
        return;
      }
      if (!isFileProtocol()) {
        var worker = null;
      ...
      loadBundle(id).then(function (bundle) {
        var src;
        try {
          src = buildBlobWorkerSource(bundle);
        } catch (e) {
          reject(e);
          return;
        }
  ...
  function load(id) {
    ...
    st.promise.catch(function () { /* 已由上层处理 */ });
    return st.promise;
  }

  // 取消指定引擎正在进行的搜索：悔棋/重开时调用。
  // 让 Worker 停掉旧思考（UCI 的 stop 指令），并清理主线程挂起的旧 seq，
  ...
  function searchPayload(fen, movetime, seq) {
    return {
      type: "SEARCH",
  ...
  function search(id, fen, movetime) {
    return new Promise(function (resolve, reject) {
      ...
      var run = function () {
        ...
        // Worker 侧就绪后也会补执行排队的 SEARCH，两边配合，
        // 保证这次走子一定有 BEST_MOVE 或 reject，不锁棋盘。
          ...
          try {
            load(id).then(function () {
              ...
              if (state(id).ready && state(id).worker && searches[seq]) {
                st.seq = seq;
                try {
                  state(id).worker.postMessage(searchPayload(fen, movetime, seq));
                } catch (e) {
                  failFast(e);
                }
              }
        ...
        st.seq = seq;
        try {
          st.worker.postMessage(searchPayload(fen, movetime, seq));
        } catch (e) {
          failFast(e);
          return;
        }
